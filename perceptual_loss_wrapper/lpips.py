...
from .lpips_backbones import *
import torch.nn
import numpy as np
from .download import download_model, download_lpips


def get_perceptual_loss(loss_fn):
    if loss_fn == 'vgg_ssl':
        download_model('simclr_vgg_phase150')  # Download the weights
        loss_fn_vgg = LPIPS(net='vgg', lpips=False, pnet_rand=True, pretrained_weights='pretrained/simclr_vgg_phase150.pt').cuda()
        loss_fn = lambda x,y: loss_fn_vgg(x, y) / 18.0
    elif loss_fn == 'lpips':
        download_lpips()  # Download LPIPS weights
        loss_fn = LPIPS(net='vgg').cuda()
    else:
...
class LPIPS(nn.Module):
    def __init__(self, pretrained=True, net='alex', version='0.1', lpips=True, spatial=False, 
        ...
        self.net = net_type(pretrained=not self.pnet_rand, requires_grad=self.pnet_tune, pretrained_weights=pretrained_weights)
        ...
        if(lpips):
            ...
            if(pretrained):
                if(model_path is None):
                    import inspect
                    import os
                    model_path = os.path.abspath(os.path.join(inspect.getfile(self.__init__), 'PerceptualSimilarity', 'weights/v%s/%s.pth' % (version, net)))
